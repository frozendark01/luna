...
```sh
docker run --rm -v ./luna.yml:/app/config/luna.yml frozendark01/luna config:print | less -N
```
...
```sh
docker run --rm frozendark01/luna secret:make
```
...
```sh
docker run --rm frozendark01/luna password:hash mysecretpassword
```
...
branding:
  custom-footer: |
    <p>Powered by <a href="https://github.com/frozendark01/luna">luna</a></p>
...
To display data from a remote server you need to have the luna Agent running on that server. You can download the agent from [here](https://github.com/frozendark01/agent), though keep in mind that it is still in development and may not work as expected. Support for other providers such as lunas will be added in the future.
